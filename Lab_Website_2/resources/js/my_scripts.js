...
function loadStatsPage() {
	...
	for(row_counter = 0; row_counter < statsTable.rows.length; row_counter++)
	{
		var buffScore;
		var oppScore;
		...
		for(col_counter = 0; col_counter < statsTable.rows[row_counter].cells.length; col_counter++)
		{
			// We're in the buff's score col
			if (col_counter == 2) {
				buffScore = parseInt(statsTable.rows[row_counter].cells[col_counter].innerHTML);
			}
			// We're in the opp score col
			if (col_counter == 3) {
				oppScore = parseInt(statsTable.rows[row_counter].cells[col_counter].innerHTML);
			}
			// we're in the last column
			if (col_counter == 4) {
				if (buffScore > oppScore) {
					statsTable.rows[row_counter].cells[col_counter].innerHTML = "CU Boulder";
					console.log("Win");
					buffWins++;
				} else {
					statsTable.rows[row_counter].cells[col_counter].innerHTML = 
					statsTable.rows[row_counter].cells[1].innerHTML;
					console.log("Loss");
					buffLosses++;
				}
			}
...
function loadPlayersPage() {
	for (var index = 0; index < players.length; index++) {
		...
		anchorTag.index = index

		anchorTag.onclick = function() { 
			switchPlayers(this.index);
		};
		anchorTag.innerHTML = players[index].name;

		document.getElementById("player_selector").appendChild(anchorTag);
	}
}
